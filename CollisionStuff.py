...
def EuclideanDists(x1, x2, y1, y2):
    return np.sqrt((x1-x2)**2 + (y1-y2)**2)

def PossibleCandidates(xs, ys, lws, rws, lambda2):
    maxlw = max(lws)
    maxrw = max(rws)
    maxw = maxlw + maxrw
    maxdist = lambda2 * maxw
    num = len(xs)
    inds = np.arange(num)
    dists = [(inds[i], inds[i+1:][np.where(EuclideanDists(xs[i], xs[i+1:], ys[i], ys[i+1:]) < maxw)][0]) for i in range(num) if len(inds[i+1:][np.where(EuclideanDists(xs[i], xs[i+1:], ys[i], ys[i+1:]) < maxw)])>0]
    return dists
        
def CollPairs(xs, ys, lws, rws,lefts, rights, lambda2):
    candidates = PossibleCandidates(xs, ys, lws, rws, lambda2)
    pairs = []
    for c in candidates:
        ind1, ind2 = c
        if CentroidIntersect(lefts[ind1], rights[ind1], lefts[ind2], rights[ind2]) == True:
...
def OneRule(left1, left2, right1, right2, lambda1, lambda2, lambda3, alpha, delta, qshiftratio):
    ...
    effectivecws = cvols**(1/3)
    leftfrac = sum(effectivecws[np.where(colxs <= colcomx)])/sum(effectivecws)
    rightfrac = 1 - leftfrac
